  ...
  @forelse ($orders as $order )  
    <div class="mx-auto max-w-2xl pt-16 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
      <div class="space-y-2 px-4 sm:flex sm:items-baseline sm:justify-between sm:space-y-0 sm:px-0">
        ...
        <p class="text-sm text-gray-600">Order placed <time datetime="2021-03-22" class="font-medium text-gray-900">{{ $order->created_at->toDateTimeString() }}</time></p>
        ...
        <div class="space-y-8">
          ...
          <div class="border-t border-b border-gray-200 bg-white shadow-sm sm:rounded-lg sm:border">
            <div class="py-6 px-4 sm:px-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:p-8">
              <div class="sm:flex lg:col-span-7">
                <div class="aspect-w-1 aspect-h-1 w-full flex-shrink-0 overflow-hidden rounded-lg sm:aspect-none sm:h-40 sm:w-40">
                  <img src="{{ $variation->getFirstMediaUrl('default','preview')}}" alt="Insulated bottle with white base and black snap lid." class="h-full w-full object-cover object-center sm:h-full sm:w-full">
                </div>
  
                <div class="mt-6 sm:mt-0 sm:ml-6">
                  <h3 class="text-base font-medium text-gray-900">
                    <a href="#">{{ $variation->product->title }}</a>
                  </h3>
                  <p class="mt-2 text-sm font-medium text-gray-900">{{ $variation->formattedPrice() }}</p>
                  <p class="mt-2 text-sm font-medium text-gray-900">Quantity: {{ $variation->pivot->quantity }}</p>
                ...
                <dl class="grid grid-cols-2 gap-x-6 text-sm">
                  <div>
                    ...
                    <dd class="mt-3 text-gray-500">
                      <span class="block">{{ $order->shippingAddress->house_number }} 
                        {{ $order->shippingAddress->address}}</span>
                      <span class="block">{{ $order->shippingAddress->city}} ({{ $order->shippingAddress->country}})</span>
                      <span class="block">{{ $order->shippingAddress->post_code }}</span>
                      <span class="block">{{ $order->shippingAddress->phone }}</span>
                    </dd>
                  </div>
                  {{-- <div>
            ...
            <div class="border-t border-gray-200 py-6 px-4 sm:px-6 lg:p-8">
              ...
              <p class="text-sm font-medium text-gray-900">Order placed at <time datetime="2021-03-24">({{ $order->placed_at->toDateTimeString() }})</time></p>
              @endif
              @if ($order->status() == "shipped_at")
              <p class="text-sm font-medium text-gray-900">Order shipped at <time datetime="2021-03-24">({{ $order->shipped_at->toDateTimeString() }})</time></p>
              @endif
              <div class="mt-6" aria-hidden="true">
                <div class="overflow-hidden rounded-full bg-gray-200">
                  ...
                  @endif
                  @if ($order->status() == "shipped_at")
                  <div class="h-2 rounded-full bg-indigo-600" style="width: calc((1 * 2 + 3) / 8 * 100%)"></div>
                  @endif --}}
        ...
        <div class="bg-gray-100 py-6 px-4 sm:rounded-lg sm:px-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:px-8 lg:py-8">
          ...
          <dl class="mt-8 divide-y divide-gray-200 text-sm lg:col-span-5 lg:mt-0">
            <div class="flex items-center justify-between pb-4">
              ...
              <dd class="font-medium text-gray-900">{{ money($order->formattedSubtotal()) }}</dd>
            </div>
            <div class="flex items-center justify-between py-4">
              <dt class="text-gray-600">ShippingType ({{ $order->shippingType->title }})</dt>
              <dd class="font-medium text-gray-900">{{ money($order->shippingType->price) }}</dd>
            </div>
            <div class="flex items-center justify-between pt-4">
              <dt class="font-medium text-gray-900">Order total</dt>
              <dd class="font-medium text-indigo-600">{{ money($order->formattedTotalAmount()) }}</dd>
            </div>
          </dl>
        </div>
      </div>
